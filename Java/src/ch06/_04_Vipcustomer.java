...
public class _04_Vipcustomer extends _04_Customer{
	...
	public _04_Vipcustomer() {//디폴트생성자를하면 초기화가 됌 예) 
							//엄마생성자에서 값을 매기면 값이 정해져있는데 
							//디폴트생성자로 엄마 클래스에서 아들클래스로 넘어오면 값이
							//0으로 초기화가됌 그래서 내가 다시 값을 매길수있음
		
		// super(); // 컴파일러가 자동으로 _04_VipCustomer()의 첫줄에 super()를 추가함.
		// super()를 호출하면 상위클래스의 디폴트 생성자가 호출된다.
		
		// 방법1. super.멤버변수호출 .. private이면 접근불가
		//super.customerGrade = "VIP"; // 엄마꺼여서 super.멤버변수 super를 써야함 엄마꺼 쓸라면
		//super.bonusRatio = 0.05;//보너스적립
		// 방법2. super(매개변수1, 매개변수2) {} 엄마클래스에서 매개변수생성자를 만들어줘야함 이렇게쓰려면
		super("vip",0.05);
		this.saleRatio = 0.1;//할인율
		this.agentID = 10011;
		System.out.println("VipCustomer() 생성자 호출");
		//생성자는 new 를 만나서 초기화해라(힙메모리에 저장)
	}
	...
	@Override// 엄마클래스에서 가져온내용을같이쓰고 아들 클래스에서 내용추가하기위해 오버라이드를 사용한다.
	public void showCustomerInfo() {
		System.out.println("고객아이디 :" + getCustomerID());
