...
public class _02_ArrayList {
	public static void main(String[] args) {
		 ...
		 * [ArrayList]
		 * 2. 선언
		 * ArrayList<데이터타입클래스> = new ArrayList<데이터타입클랫>();
		 * 
		 * 다형성적용
		 ...
		 * 
		 * 4. 추가 : list.add(추가할 위치index,값); // 지정한 인덱스에 값을 추가하겠다.그 뒤값은 밀려난다
		 * 			list.add(값); // 앞에서부터 순서대로 추가
		 * 5. 교체 : list.set(교체할 위치 index,값); // 지정한 인덱스의 값을 변경하겠다그 뒤값은 그대로이다.
		...
		list.add(4, "대방어회");// 인덱스를 통해 값을 추가 
		list.set(5,"망고"); //set = 녹두전 -> 망고로 교체 
		//방법1. for
		for(int i = 0; i < list.size(); i++) {
			System.out.println(list.get(i));
		}
		
		System.out.println("향상된 for문지방구린내부자석박지렁이재동대문서대문자동문지기차표범생선풍기성용건장소금지름길동무");
		//방법2. 향상된 for문
		for(String al : list) {
		...
		while(iterator.hasNext()) {//ArrayList의 값이 존재하는지 체크 / hasNext()타입이 boolean
			...
			String str = (String) iterator.next(); // return 타입이 Object
			//Object str = iterator.next();
			System.out.println(str + " ");
			// 무슨값인지모르니깐 Object로 받는다
		}
	}		
}              