...
public class _06_TypeCasting {
	public static void main(String[] orgs) {
		 ...
		 * 	 자료형이 다른 변수에 값을 할당하는 경우 형변환을 생각할 수 있다.
		 * 
		 * 	 데이터 큰 타입 = 데이터 작은 타입; (ex: float f = 10 // f에 10.0이 대입);
		 * 	 자료형이 다른 값을 연산하는 경우; (ex : double d = 10.0 + 20; // d = 10.0 + 20.0)
		 * 
		 * 2. 강제적인 형변환(축소 형변환, 명식적 형변환)
		 * 	 명시적으로 형변환하는 것을 캐스팅(Casting)이라고 한다.
		 *   Casting은 생략할 수 없으며, 생략시 컴파일 에러가 발생하므로 반드시 명시적으로 형변환해야 한다.
		 *   강제로 형변환하므로 데어터 손실 위험이 있다.
		 *   
		 *   
		 *   데어터 작은 타입 = (작은 타입)데이터 큰타입;
		 */
	System.out.println("형변환-1 예제===");
	
	int j = (int) 10.9; // 강제 형변환 = int j = 10.9;-> int j = (int) 10.9;
	System.out.println("j : " + j);// 강제 형변환(축소 형변환) : 데이터 작은 타입 = (작은 타입)데이터 큰타입;
	
	double d = 10;
	System.out.println("d : " + d); // 자동 형변환(확대 형변환) : 데이터가 큰 타입 = 디어타 작은 타입;
	...
	/*
	 * 형변환-2.
	 * 3. int(4바이트)보다 크기가 작은 자료형은 int형으로 형변환후 계산
	 * 	ex) byte + short -> int + int
	 * 
	 * 4. 두개의 피연산자 중 표현범위가 큰 쪽으로 맞춰서 형변환 후 계산
	 */
	...
	ff =(double) 5 / 4; // 4번 적용.. double / int -> double / double -> 5.0 / 4.0 -> 1.25
	System.out.println("ff : " + ff);
	
	ii = (int) (2.5 + 3.3);  // 형변환보다 ()안에 계산이 우선.. ii = (int)(5.8);
	System.out.println("ii : " +ii);
	
	//ii = (int)2.5 + 3.3; //->컴파일 에러.. 앞에는 인트형이고 뒤에넌 더블형이여서 위에나아래로 바꿔줘야함
	...
	long c3 = gg * hh; // long형으로 형변환하기 전부터 int형 초과
	System.out.println("c3 : "+ c3); // c2 : -1454759936
	
    long c4 = (long)gg * hh; //long * int -> long * long -> long
	System.out.println("c4 : "+ c4); // c4 : 20000000000000
