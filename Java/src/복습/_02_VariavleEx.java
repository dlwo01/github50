...
public class _02_VariavleEx {
	public static void main(String[] args) {
		/*
		 * 메소드명, 번수명은 소문자로 시작한다. (cf. 글래스명은 대문자로 시작한다.)
		 * 변수 : 데이터를 담는 그릇이며, 데이터의 저장과 참조를 위해ㅔ 메모리 공간을 할당받는다.
		 *      반드시 먼저 선언 : 컴파일러는 변수의 자료형에 맞는 기억공간을 미리 확보
		 *      선언 : 자료형이 먼저오고 가듬에 변수형이온다; 예)int a = 0;
		 *      선언과 동시에 할당 : 자료형이 오고 변수형이온다 = 초기와 예) int a = 8;
		 *      
		 *      
		 *1. 기본자료형(Prinitive Type) : 실제값이 저장
		 *   정수 : byte(1바이트) -> short(2바이트) -> int(4바이트) ㅡ> long(8바이트)
		 ...
		 *   
		 *   
		 *   2. 참조형(Reference Type) : 실제 객체를 가리키느 주소값 저장
		 *     클래스,인터페이스,배열
		 *     
         *     
         * 문자열 : String 클래스
	...
	float f = 3.14f; // float형은 값뒤에 f f를 붙여준다.
